...
import { reactivateSubscription } from "@/services/billing/billing.service";
...
export async function POST(request: Request) {
  try {
    ...
    const result = await reactivateSubscription(parsed.data);
    if (!result.ok) {
      return jsonFail(
        result.code ?? "UNKNOWN",
        result.message ?? "Unable to reactivate subscription.",
    ...
    return jsonFail("UNKNOWN", "Unable to reactivate subscription.", 500);
  }
}
